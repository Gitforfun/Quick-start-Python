...
while answer !='q':
	...
	if (answer == "y"):
		...
		if (do == 1):
			...
			flag = 0													# флаг для цикла while. Указывает было ли правильно выполнено условие внутри while-if
			while flag == 0:	
				...
				if os.path.isfile(file_list[i-1]):							# Проверка выбора файла и не папки
					...
					print (file_list[i-1], ' - является каталогом, а не папкой. Выберете файл. ')
			...
			for i in range (0, len(file_list)):
				print ("	", i+1, ".	", file_list[i])
			print ('!!!{}, Вы уверены, что хотите удалить все бэкапы?'.format(name))		
