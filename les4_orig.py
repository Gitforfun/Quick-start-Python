...
def delete_duplicates(select_dir):
	count = 0
	file_list = os.listdir(select_dir)
	for f in file_list:
		fullname = os.path.join(select_dir, f)  # \ - разделитель в win, / - разд.в unix
...
while answer != 'q':
	...
	if answer == "y":
		...
		if do == 1:
			...
			print("Кол-во удаленных файлов: ", delete_duplicates(dirname), " Перезагрузка.")
		else:
